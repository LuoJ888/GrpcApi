...
class XlwtSeting(object):
    ...
    "horz": "CENTER", "vert": "CENTER"}
        sizes = [10, 11, 11, 11, 11, 30, 60, 80, 11, 11, 11, 11, 11, 11, 11]
    ...
    def alignments(wrap=1, **kwargs):
        """设置单元格的对齐方式，
        ：接收一个对齐参数的字典{
    "horz": "CENTER", "vert": "CENTER"}horz（水平），vert（垂直）
...
class XmindToXsl(XlwtSeting):
    ...
    def write_excel(self, performer=''):
        """生成excel文件的方法"""
        row0 = ['用例目录', "用例名称", '需求ID', '前置条件', '需求名称', '测试用例名称', '用例步骤',
                '预期结果', '用例类型', '用例状态', '用例等级', '创建人', '全量', '预发布', '发布']
                        ...
                        for k in range(suit_num):
                            ...
                            if c1 != 0:
                                for n in range(c1):
                                    ...
                                    self.worksheet.write(x, 6, step, style)  # 写入执行步骤
                                    self.worksheet.write(x, 7, exce, style)  # 写入预期结果
                                    self.worksheet.write(x, 11, performer, style)  # 写入执行人
                                    self.worksheet.write(x, 0, '6_动态', style)  # 用例目录
                                    self.worksheet.write(x, 3, 'case step', style)  # 前置条件
                                    self.worksheet.write(x, 8, '功能测试', style)  # 用例类型
                                    self.worksheet.write(x, 9, '正常', style)  # 用例状态
                                    self.worksheet.write(x, 10, 'P0', style)  # 用例等级
                                    self.worksheet.write(x, 1, f'id_{x}', style)  # testcase id
                                mod = self.xmind_title(test_module)  # 测试需求名称
                                case = self.xmind_title(test_case)  # 测试用例名称
                                self.worksheet.write_merge(x - c1 + 1, x, 4, 4, mod, style)  # 写入测试需求名称
                                self.worksheet.write_merge(x - c1 + 1, x, 5, 5, case, style)  # 写入测试用例名称
                            else:
