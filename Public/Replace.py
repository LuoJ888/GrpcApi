...
class MainUI(object):
    ...
    def __init__(self, title="Replace", geometrysize="350x250", geometry="+800+350"):
    ...
    def get_value(self):
        ...
        path = self.path.get()
        per = self.person.get()
        print(f"地址：{path}，执行人员：{per}")
        regvalue = '.*\.xmind$'
        xmind_reg = re.match(regvalue, path)
        if xmind_reg:
            # xmind转换成xls
            xmind_to_xls = XmindToXsl(path)
            xmind_to_xls.write_excel(performer=per)
        else:
    ...
    def create_widgets(self):
        ...
        way_button = tkinter.Button(self.top, text="路径选择", command=self.select_path,bg='light blue')
